...
int main(int argc, char const *argv[])
{
	...
	std::vector<double> vTs;
	std::vector<std::vector<double>*> vve;
	std::vector<std::vector<double>*> vvE;
	std::vector<std::vector<double>*> vvv;
	std::vector<std::vector<double>*> vvZ;
	std::vector<std::vector<double>*> vvrhoe;
	std::vector<std::vector<double>*> vvp;
	std::vector<std::vector<double>*> vvrho;
	std::vector<std::vector<double>*> vvT;
	std::vector<std::vector<double>*> vvZvsVT;
	std::vector<std::vector<double>*> vvUinvsVT;
	...
	for (double time = 0; time < endt; time+=dt)
	{
		...
		for (int i = 1; i < nmeshpoints+1 ; ++i)
		{
			...
			for (int j = 1; j >=0 ; --j)
			{
				...
				double gammal = (Al*Bl+vl*Cl)*rhol;
				double dpl = 2*PI*Al*Al*Cl*rhol+vl*gammal;
				double del = (4*PI*pow(Al,3)*Bl+5*PI*Al*Al*vl*Cl)*rhol+0.5*vl*vl*gammal;
				double bouncebackrate = i-!j==nmeshpoints?1:std::min(pow(vvrho.back()->at(i-!j)/mat.rho,2./3),1.);
				double rrmol = pow(mat.rho/rhol,1./3);
				double r0 = pow(mat.n/N_A/mat.rho,1./3);
				double pmol = mat.YoungsModulus/6*(pow(rrmol,12)-pow(rrmol,6))/rrmol/r0;
				...
				if(i==nmeshpoints){
					...
					if(j==1){
							phipl[phipl.size()-1]+=dpl*dt+pmol;//*(1-bouncebackrate)+1*dpl*dt*bouncebackrate;
						...
						phipr[phipr.size()-1]-=dpl*dt-pmol;//*(1-bouncebackrate)+1*dpl*dt*bouncebackrate;
						//phipl[phipl.size()-1]+=dpl*dt*(1-bouncebackrate)+2*dpl*dt*bouncebackrate;
						phie[phie.size()-1]-=del*dt*(1-bouncebackrate);
						phi[phi.size()-1]-=gammal*dt*(1-bouncebackrate);
					}
				}
				//if(abs(i-matthickness/dx)<10 && time >= 181e-15 && (i==100||i==101))
				//	printf("Al=%e, Bl=%e, Cl=%e, v=%e, gammal=%e, del=%e, dpl=%e, i=%d, j=%d\n", Al, Bl, Cl, vvv.back()->at(i-j)/rhol/dx, gammal*dt,del*dt,dpl*dt,i,j);
			}
				//if(abs(i-matthickness/dx)<10)
		...
		for (int i = 0; i < nmeshpoints-1; ++i)
		{
			...
			//printf("dp1=%e, dp2=%e, dp=%e, drho=%e\n", phipl[i]-phipr[i], -phipl[i+1]+phipr[i+1], dv, tmprho+(phi[i]-phi[i+1])/dx-vvrho[0]->at(i+1));
...
//			if(dv>0) exit(0);
			...
			//if(tmprho!=0 && abs(i-matthickness/dx)<10 && time >= 181e-15) printf("rho=%e, E=%e, de=%e, dv=%e, v=%e\n", tmprho, tmpvE->back(), de, dv, (vvv.back()->at(i+1)+dv)/tmpvrho->back());
			de-=dep;
			...
			if(tmpvrho->back()>1e-200){
				...
				if(tmprho>1e-20){
					...
					de+=experiment.laserStrength(time-2*experiment.drilling.pulseWidth,0)*dt*(1-maxreflect)*dx*alpha*Ileft;
...
//		plot.plotSingle("rho"+std::to_string(time*1e15),vx,*tmpvrho,"x/m","rho"+std::to_string(time*1e15)+".png");
		...
		if(round((time-recorded)*1e15)>=0) {
			...
			if(time>1000e15) recorded+=10*recorddt;
			if(time>10000e15) recorded+=100*recorddt;
			else recorded+=recorddt;
